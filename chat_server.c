...
/*finds out if the username exists in the list
and also checks if its the correct admin deleting*/
struct client_sock* kickUserOut(char * username, struct client_sock *admin, struct client_sock *clients){
    struct client_sock * pointer = clients;
    struct client_sock * temp = NULL;
    if (admin == clients){
        while (clients != NULL){
            if (strcmp(clients->username+1, username) == 0){
                temp = clients;
            }
            clients = clients->next;
        }
...
/*checks if a username is invalid and not satisfying the conditions
1) username is SERVER
2) oversized name
3) already taken
...
int validUserName(struct client_sock* clients, struct client_sock* curr){
    ...
    if (strlen(curr->username+1) > MAX_NAME || strcmp("SERVER", curr->username+1) == 0){
        return 1;
    }
    struct client_sock * temp = clients;
    while (clients != NULL && clients->username != NULL){
        if (clients != curr){
            if (strcmp(clients->username+1, curr->username+1) == 0){
                clients = temp;
                return 1;
            }
        }
    ...
    do {
        ...
        while (curr) {
            ...
            if (client_closed == 0 && curr->username == NULL) {
                if (set_username(curr) || validUserName(clients, curr)) {
                    printf("Error processing user name from client %d.\n", curr->sock_fd);
                    client_closed = 1; // Disconnect the client
                    char buf[44];
                    strncat(buf, "1SERVER Username invalid or already taken.\0", 42);
                    write_buf_to_client(curr, buf, strlen(buf));
                    buf[0] = '\0';
                }
                else {
                    printf("Client %d user name is %s.\n", curr->sock_fd, curr->username+1);
                }
            }
            ...
            while (client_closed == 0 && !get_message(&msg, curr->buf, &(curr->inbuf))) {
                ...
                if (num == 48){
                    struct client_sock*temp = kickUserOut(msg+1,curr, clients);
                    if (temp != NULL){
                        FD_CLR(temp->sock_fd, &all_fds);
                        close(temp->sock_fd);
                        printf("Client %d disconnected\n", temp->sock_fd);
                        assert(remove_client(&temp, &clients) == 0);
                    }
                    break;
                }
                ...
                strncat(write_buf, msg+1, MAX_USER_MSG); //didn't add  + 1 for proto_code
                free(msg);
                ...
                while (dest_c) {
                    ...
                    strcpy(buf, write_buf);
                    if (dest_c != curr) {
                        int ret = write_buf_to_client(dest_c, buf, data_len);
                        if (ret == 0) {
                            printf("Sent message from %s (%d) to %s (%d).\n",
                                curr->username+1, curr->sock_fd,
                                dest_c->username+1, dest_c->sock_fd);
                        }
                        else {
